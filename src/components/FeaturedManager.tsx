...
export const FeaturedManager: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const featuredData = await CRMService.getAllContent({ type: 'featured' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedFile) {
        const publicUrl = await StorageService.uploadImage(selectedFile, 'featured');
      ...
      await loadData();
      resetForm();
      alert('Featured item saved successfully!');
    } catch (error) {
      console.error('Error saving featured item:', error);
      alert(`Error saving featured item: ${error instanceof Error ? error.message : 'Please try again.'}`);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this featured item?')) {
      try {
        ...
        if (item?.file_path && item.file_path.includes('supabase.co')) {
          try {
            await StorageService.deleteImage(item.file_path);
          } catch (storageError) {
        ...
        alert('Error deleting featured item. Please try again.');
      }
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">Featured On</h2>
          <p className="text-gray-600 mt-2">Manage publications and platforms that featured your work</p>
        </div>
        <button
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
        {featured.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 text-lg mb-2">No featured items yet</p>
            <p className="text-gray-400 text-sm">Add publications or platforms that featured your work</p>
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
            {featured.map((item) => (
              ...
              >
                ...
                <p className="text-sm text-center mt-2 text-gray-700 truncate">{item.title}</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Marie France Asia, BrideStory"
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g., Marie France Asia logo"
                    className="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                ...
                <div className="flex gap-4 pt-4">
                  ...
                  >
                    <Save size={20} />
                    {editingId ? 'Update' : 'Add'} Featured Item
                  </button>
                  <button
                    type="button"
                    onClick={resetForm}
